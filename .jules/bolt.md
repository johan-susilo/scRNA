...
## 2025-03-05 - Avoid O(N) Subsetting Inside Loops with SplitObject
**Learning:** Repetitive calls to `subset(seurat_obj, metadata == id)` inside `for` loops create severe computational bottlenecks in Seurat objects because each subset operation parses and copies large expression matrices.
**Action:** Extract subset logic outside the loop using `SplitObject(seurat_obj, split.by = "metadata")` to pre-partition the object, reducing subset operations from O(n) to O(1) loop lookups.
