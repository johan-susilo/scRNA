#!/usr/bin/env Rscript
# usage : Rscript ./myPreprocessing.R -s data1_folder,data2_folder,... -ident1 data1_nature,data2_nature,... -identi2 data1_ID,data2_ID,...
# ex    : Rscript ./myPreprocessing.R -s sample1,sample2,sample3,... -ident1 1_Healthy_skin,2_Acute_skin,3_Chronic_skin -ident2 HC131,HC244,HC258...
#above commandline is just example, modified it due to your usage!
#nohup Rscript ./lessPreprocessing.R -f input.tsv &
Sys.time()
...
if (length(args) < 1) {
  stop("Please provide a TSV file using -f.")
}
...
while (i <= length(args)) {
  if (args[i] == "-f") {
    if (i + 1 <= length(args) && substr(args[i + 1], 1, 1) != "-") {
      ...
      stop("Please provide a TSV file after -f")
    }
  }
  i <- i + 1
}
...
sample_list <- list()
sample_names <- samples_df$sample_names #to read all sample names from tsv file
...
for (rds_file in rds_files) {
  ...
  sample <- gsub("_processed\\.rds$", "", basename(rds_file))

  # Create a variable name by converting the sample name to lowercase
  variable_name <- tolower(sample)
  print(variable_name)	
 ...
 for (i in seq_len(nrow(samples_df))) {
      sample_name <- samples_df$sample_names[i] #read each sample name from tsv file
      ...
      data_dir <- paste0("/mnt/80T/johan/data/liver_R/data/",sample_name)
      print(data_dir)
      ...
      # Check if output are set, then process each
      if(file.exists(output_rds)) {
          print(paste("Loading preprocessed:", sample_id))
      ...
      vln_data <- VlnPlot(seur_obj, features = c("nFeature_RNA", "nCount_RNA", "percent.mt"), pt.size = 0.1, combine = FALSE)
      #Adjust y-axis per feature
      vln_data[[1]] <- vln_data[[1]] + scale_y_continuous(limits = c(0, 10000))  # nFeature_RNA y-axis
      vln_data[[2]] <- vln_data[[2]] + scale_y_continuous(limits = c(0, 40000))  # nCount_RNA y-axis
      vln_data[[3]] <- vln_data[[3]] + scale_y_continuous(limits = c(0, 100))    # percent.mt y-axis
      #combined 3 violin plot in one figure
      combined_plot <- patchwork::wrap_plots(vln_data[[1]], vln_data[[2]], vln_data[[3]], ncol = 1)
      ...
      saveRDS(seur_obj, file = output_rds)
      print(paste("Saved processed:", sample_id))
      }
      sample_list[[sample_ident]] <- seur_obj
      }     
...
print(table(Idents(TN.combined), TN.combined$orig.ident2))


#CellNumber
CellNumber <- table(Idents(TN.combined), TN.combined$orig.ident1)
write.csv(CellNumber , file = "./output/CellNumber_bygroup.csv")
CellNumber_sample <- table(Idents(TN.combined), TN.combined$orig.ident2)
write.csv(CellNumber_sample , file = "./output/CellNumber_bysample.csv")
...
dev.off()
pdf ("./output/TNcombined_umap_labelT_splitsample.pdf", width = 15, height = 15)
...
Cellproportion_sample <- table(Idents(TN.combined), TN.combined$orig.ident2)
Cellproportion_sample <- round(sweep(Cellproportion_sample,MARGIN=2, STATS=colSums(Cellproportion_sample), FUN = "/")*100,2)
write.csv(Cellproportion_sample, file = "./output/Cellproportion_bysample.csv", row.names = T)
...
proportion_plot <- ggplot(Cellproportion, aes(x = Var2, y = Freq, fill = Var1)) + theme_bw(base_size = 15) +
  ...
  theme(legend.title = element_blank())+ scale_fill_manual(values = mycolor)
