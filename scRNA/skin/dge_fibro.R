...
for (cell_type in cell_types) {
  ...
  rds_path <- file.path(base_dir, cell_type, "processed", paste0(cell_type, "_annotated_final.rds"))
  ...
  pb_sub <- AggregateExpression(sub_obj, assays = "RNA", 
                                group.by = c("SingleR_label", "orig.ident2", "Condition"), 
                                return.seurat = FALSE)$RNA
  
  clusters <- levels(factor(sub_obj$SingleR_label))
  all_sub_cols <- colnames(pb_sub)
  
  for (cluster_id in clusters) {
    
    # NEW FIX: Seurat replaced our "_" with "-" in the matrix columns. 
    # We must format our search string to match!
    safe_cluster_id <- gsub("_", "-", cluster_id) 
    
    # Now use safe_cluster_id to find the columns
    cluster_cols <- all_sub_cols[grepl(paste0("^g?", safe_cluster_id, "_"), all_sub_cols)]
    
    if (length(cluster_cols) == 0) { 
      message(paste("Skipping", cluster_id, "- No columns found."))
      next 
    }
    ...
    meta_sub <- data.frame(pseudobulk_id = cluster_cols) %>%
      mutate(
        condition = str_extract(pseudobulk_id, "(PMH|Healthy)$"),
        sample_id = str_extract(pseudobulk_id, "(?<=_).*?(?=_(PMH|Healthy)$)")
      )
    rownames(meta_sub) <- meta_sub$pseudobulk_id
    meta_sub$condition <- factor(meta_sub$condition, levels = c("Healthy", "PMH"))
    
    # Use our updated function with fitType='local'
    run_and_save_deseq2(
      counts_matrix = counts_sub, 
      meta = meta_sub, 
      comparison_name = paste0(cell_type, "_", cluster_id), 
      out_dir = out_dir, 
      title_prefix = paste(toupper(cell_type), "-", cluster_id)
    )
  }
