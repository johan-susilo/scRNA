...
# ⚡ Bolt: Use SplitObject to avoid O(n) subsetting overhead in loops
sample_list <- SplitObject(FB.subgroup, split.by = "orig.ident2")
for (sample_id in names(sample_list)) {
  sample_obj <- sample_list[[sample_id]]
...
if(length(available_mucin) > 0) {
  ...
  # ⚡ Bolt: Use SplitObject to avoid O(n) subsetting overhead in loops
  sample_list_mucin <- SplitObject(FB.subgroup, split.by = "orig.ident2")
  for (sample_id in names(sample_list_mucin)) {
    sample_subset <- sample_list_mucin[[sample_id]]
...
# ⚡ Bolt: Use SplitObject to avoid O(n) subsetting overhead in loops
sample_list_macro <- SplitObject(FB.subgroup, split.by = "orig.ident2")
for (sample_id in names(sample_list_macro)) {
  sample_obj <- sample_list_macro[[sample_id]]
