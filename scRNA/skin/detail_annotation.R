...
# BOLT OPTIMIZATION: Use SplitObject once instead of repeated subset() calls in multiple loops
# This prevents scanning the entire Seurat object repeatedly and improves performance
FB.subgroup.list <- SplitObject(FB.subgroup, split.by = "orig.ident2")

for (sample_id in names(FB.subgroup.list)) {
  sample_obj <- FB.subgroup.list[[sample_id]]
...
if(length(available_mucin) > 0) {
  ...
  for (sample_id in names(FB.subgroup.list)) {
    sample_subset <- FB.subgroup.list[[sample_id]]
