...
# [BOLT OPTIMIZATION] - Performance boost: Unconditionally partition the Seurat object once outside the loop using SplitObject() to prevent O(N) operations while avoiding stale data bugs.
split_FB_subgroup <- SplitObject(FB.subgroup, split.by = "orig.ident2")
for (sample_id in names(split_FB_subgroup)) {
  sample_obj <- split_FB_subgroup[[sample_id]]
...
if(length(available_mucin) > 0) {
  ...
  split_FB_subgroup <- SplitObject(FB.subgroup, split.by = "orig.ident2")
  for (sample_id in names(split_FB_subgroup)) {
    sample_subset <- split_FB_subgroup[[sample_id]]
