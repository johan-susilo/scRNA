...
# Bolt Optimization: Split Seurat object once outside loop instead of subsetting inside loop
fb_split_by_sample <- SplitObject(FB.subgroup, split.by = "orig.ident2")
for (sample_id in names(fb_split_by_sample)) {
  sample_obj <- fb_split_by_sample[[sample_id]]
...
if(length(available_mucin) > 0) {
  ...
  # Bolt Optimization: Split Seurat object once outside loop instead of subsetting inside loop
  fb_split_by_sample_mucin <- SplitObject(FB.subgroup, split.by = "orig.ident2")
  for (sample_id in names(fb_split_by_sample_mucin)) {
    sample_subset <- fb_split_by_sample_mucin[[sample_id]]
...
# Bolt Optimization: Split Seurat object once outside loop instead of subsetting inside loop
fb_split_by_sample_macro <- SplitObject(FB.subgroup, split.by = "orig.ident2")
for (sample_id in names(fb_split_by_sample_macro)) {
  sample_obj <- fb_split_by_sample_macro[[sample_id]]
