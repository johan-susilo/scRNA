...
# ⚡ Bolt Optimization: Use SplitObject outside the loop instead of O(n) subset() calls inside the loop.
split_by_sample_3B <- SplitObject(FB.subgroup, split.by = "orig.ident2")
for (sample_id in names(split_by_sample_3B)) {
  sample_obj <- split_by_sample_3B[[sample_id]]
...
# ⚡ Bolt Optimization: Explicitly remove list object to prevent OOM failures
rm(split_by_sample_3B)
gc()
...
if(length(available_mucin) > 0) {
  ...
  # ⚡ Bolt Optimization: Use SplitObject outside the loop instead of O(n) subset() calls inside the loop.
  split_by_sample_mucin <- SplitObject(FB.subgroup, split.by = "orig.ident2")
  for (sample_id in names(split_by_sample_mucin)) {
    sample_subset <- split_by_sample_mucin[[sample_id]]
...
# ⚡ Bolt Optimization: Use SplitObject outside the loop instead of O(n) subset() calls inside the loop.
split_by_sample_D <- SplitObject(FB.subgroup, split.by = "orig.ident2")
for (sample_id in names(split_by_sample_D)) {
  sample_obj <- split_by_sample_D[[sample_id]]
