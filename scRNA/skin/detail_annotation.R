...
# OPTIMIZATION: Avoid O(n) subset() calls inside loop by splitting object once
fb_split_by_sample_3b <- SplitObject(FB.subgroup, split.by = "orig.ident2")

for (sample_id in names(fb_split_by_sample_3b)) {
  sample_obj <- fb_split_by_sample_3b[[sample_id]]
...
# OPTIMIZATION: Prevent OOM by explicitly removing split list and garbage collecting
rm(fb_split_by_sample_3b)
gc()
...
if(length(available_mucin) > 0) {
  ...
  # OPTIMIZATION: Avoid O(n) subset() calls inside loop by splitting object once
  fb_split_by_sample_6 <- SplitObject(FB.subgroup, split.by = "orig.ident2")

  for (sample_id in names(fb_split_by_sample_6)) {
    sample_subset <- fb_split_by_sample_6[[sample_id]]
...
# OPTIMIZATION: Avoid O(n) subset() calls inside loop by splitting object once
fb_split_by_sample_9 <- SplitObject(FB.subgroup, split.by = "orig.ident2")

for (sample_id in names(fb_split_by_sample_9)) {
  sample_obj <- fb_split_by_sample_9[[sample_id]]
